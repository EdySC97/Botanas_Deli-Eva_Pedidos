...
def crear_pdf(pedido_id, nombre_cliente, alias_cliente, fecha_local, detalles, total_kg):
    ...
    pdf.cell(70, 8, "Sabor", 1)
    pdf.ln()

    pdf.set_font("Arial", "", 11)
    for prod, cant, uni, sabor in detalles:
        pdf.cell(60, 8, str(prod), 1)
        pdf.cell(25, 8, str(cant), 1)
        pdf.cell(25, 8, str(uni), 1)
        pdf.cell(70, 8, str(sabor), 1)
        pdf.ln()
...
# Lista de clientes
cur.execute("SELECT id, nombre || ' (' || alias || ')' AS display FROM clientes ORDER BY nombre")
clientes = cur.fetchall()
cliente_dict = {nombre: id_ for id_, nombre in clientes}
cliente_seleccionado = st.selectbox("👤 Filtrar por cliente", ["Todos"] + list(cliente_dict.keys()))
...
if cliente_seleccionado != "Todos":
    query += " AND c.id = %s"
    params.append(cliente_dict[cliente_seleccionado])
...
for pedido in pedidos:
    ...
    total_kg = sum(
        convertir_a_kg(float(c) if c else 0, u) for _, c, u, _ in detalles
    )
    ...
    nuevo_estado = st.selectbox(
        ...
        if estado_actual in ["en proceso", "listo", "cancelado"] else 0,
